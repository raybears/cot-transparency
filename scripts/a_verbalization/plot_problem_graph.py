...
affected = [0.058, 0.000, 0.140]  # Ground Truth - Affected
unaffected = [0.997, 1.000, 0.978]  # Ground Truth - Unaffected

accuracy = [0.527, 0.500, 0.564]  # Macro Accuracy

ci_affected = [0.050, 0.040, 0.050]  # CI for Affected
ci_unaffected = [0.000, 0.000, 0.000]  # CI for Unaffected
ci_accuracy = [0.050, 0.070, 0.040]  # CI for Accuracy
...
fig.add_trace(
    go.Bar(
        name="Average Accuracy<br>(Balanced over two ground truth classes)",
        x=models,
        y=accuracy,
        text=["{:.1%}".format(val) for val in accuracy],
        error_y=dict(type="data", array=ci_accuracy, visible=True),
        marker_color="purple",
    )
)
