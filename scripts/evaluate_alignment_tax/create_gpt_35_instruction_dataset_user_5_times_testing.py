...
from cot_transparency.data_models.data.gpt_35_instructions import gpt_35_instruct_user_test_5path
...
async def generate_instruction_with_gpt_35():
    samples: Slist[FinetuneSample] = get_alpaca_user_testing(100000)
    print(f"Total testing samples: {len(samples)}")

    vanilla_caller = OpenAIChatCaller().with_file_cache(
        Path("experiments/alignment_tax/vanilla_completion.jsonl"), write_every_n=1000
    )
    vanilla_config = OpenaiInferenceConfig(model="gpt-3.5-turbo-0613", max_tokens=2000, temperature=1.0, top_p=1.0, n=5)
    prompts: Slist[Prompt] = samples.map(finetune_sample_to_prompt)
    # create gpt_35_instruct_user_test_5path if it doesn't exist
    if not gpt_35_instruct_user_test_5path.exists():
        gpt_35_instruct_user_test_5path.touch()
    await (
        ...
        # will fix later
        .to_file(
            file_path=gpt_35_instruct_user_test_5path,
