...
def modal_agreement_for_task_hash_if_modally_wrong(unique_over_model: Slist[TaskOutput]) -> float:
    # should already be unique over a model
    assert unique_over_model.length > 0, "grouped_by_task_hash should not be empty"
    grouped_by_qn = unique_over_model.group_by(lambda task: task.task_spec.task_hash)
    filtered = grouped_by_qn.filter(lambda tupl: modal_is_wrong(tupl[1]))
    # now we have a list of tasks for each task_hash
    # we want to calculate the modal agreement for each task_hash
    modal_agreements: Slist[float] = filtered.map_2(lambda task_hash, tasks: modal_agreement(tasks))
    # take the mean of the modal agreements
    average = modal_agreements.average()
    if average is None:
        print("breakpooint")
...
def bootstrap_over_formatters(name: str, items: Slist[TaskOutput], name_override: Mapping[str, str]) -> PlotDots:
    ...
    return PlotDots(name=name, acc=acc)


def calculate_modal_agreement(name: str, items: Slist[TaskOutput]) -> PlotDots:
    # non bootstrap version
    # run modal_agreement_for_task_hash
    modal_agreement = modal_agreement_for_task_hash(items)
    return PlotDots(name=name, acc=AccuracyOutput(accuracy=modal_agreement, error_bars=0, samples=items.length))
