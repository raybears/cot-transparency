...
def resync_wandb_run(
    wandb_run_id: str, job_results: FinetunedJobResults, project: str
) -> None:
...
def get_all_runs_with_missing_finetune_model(project: str) -> list[Any]:
    ...
    return [
        run
        for run in runs
        if "finetune_model_id" not in run.config or "trained_tokens" not in run.config
    ]
    ...
    for project_dir in ["consistency-training", "prompt_sen_experiments"]:
        ...
        for run in runs:
            if "finetune_job_id" in run.config:
                try:
                    finetune_job = openai.FineTuningJob.retrieve(
                        run.config["finetune_job_id"]
                    )
                    if finetune_job["status"] == "succeeded":
                        job_results: FinetunedJobResults = (
                            FinetunedJobResults.model_validate(finetune_job)
                        )
                        resync_wandb_run(
                            wandb_run_id=run.id,
                        ...
                        print(
                            f"Resynced {run.name} with finetune_model_id {run.config['finetune_job_id']}"
                        )
                    else:
                        print(
                            f"Skipping {run.name} because finetune_job status is {finetune_job['status']}"
                        )
