...
def dump_correct_data(cot_data: bool, exp_dir: str, model: str) -> None:
    ...
    selected_formatter = (
        ZeroShotCOTUnbiasedWithNoneFormatter.name() if cot_data else ZeroShotUnbiasedWithNoneFormatter.name()
    )
    ...
    if cot_data:
        write_jsonl_file_from_basemodel(
            path=Path(f"data/training_cots/{model_file_name}_unfiltered_none_options.jsonl"),
            basemodels=jsons_tasks,
        )
    else:
        write_jsonl_file_from_basemodel(
            path=Path(f"data/training_non_cots/{model_file_name}_unfiltered_none_options.jsonl"),
            basemodels=jsons_tasks,
        )
