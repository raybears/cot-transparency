...
async def train_and_run(anti_bias_samples: int) -> None:
    ...
    sampler: ResampleIfNeededSampler | NFormatsPerQuestionSampler = (
        ResampleIfNeededSampler(formatter_options=FormatterOptions.suggested_answer_cot_only)
        if n_formats_per_question > 1
        else NFormatsPerQuestionSampler(
            n_formats_per_question=1, formatter_options=FormatterOptions.suggested_answer_cot_only
        )
    )
    ...
    model = fine_tune_with_bias_augmentation(
        ...
        override_instruct_samples=95_000,
