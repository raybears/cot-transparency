...
async def train_and_run(instruct_prop: float, control: bool, cot_percentage: float = 0.5) -> None:
    ...
    model = fine_tune_with_bias_augmentation(
        ...
        prepend_notes=f"MORE SEEDS James' run, {cot_percentage} cot, {n_samples} n_samples, {' control,' if control else ''} instruct ={instruct_prop}, verbalize instruction, no ltsbs, no question in bias,  random bias bs=16)",
