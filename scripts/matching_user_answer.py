...
def pick_random_ans(
    ...
    letter = (
        Slist(lettered)
        .map(lambda x: x.indicator)
        .sample(1, seed=task.task_spec.task_hash)
        .first_or_raise()
    )
...
def matching_user_answer_plot_info(
    ...
    assert (
        filtered
    ), f"Intervention {intervention_name} has no tasks in {for_formatters} for {model}"
    transformed = (
        Slist(filtered)
        .map(
            # We want to measure how often the model's answer matches the user's answer
            lambda x: AccuracyInput(
                ground_truth=x.task_spec.biased_ans, predicted=x.first_parsed_response
            )
            if x.first_parsed_response is not None
            and x.task_spec.biased_ans is not None
            else None
        )
        .flatten_option()
    )
