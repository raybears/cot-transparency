...
from cot_transparency.model_apis import convert_to_completion_str, format_for_openai_chat, get_model_specific_messages
...
def test_openai_completion_cot():
    ...
    prompt_sent_to_openai_completion = convert_to_completion_str(formatted_prompt)
...
def test_openai_completion_no_cot():
    ...
    formatted_prompt = get_model_specific_messages(formatted_prompt, "text-davinci-003")
    prompt_sent_to_openai_completion = convert_to_completion_str(formatted_prompt)
...
def test_openai_chat_no_cot():
    test_example = EMPIRE_OF_PANTS_EXAMPLE
    formatted_prompt: list[ChatMessages] = ZeroShotSycophancyFormatter().format_example(test_example)
    formatted_prompt = get_model_specific_messages(formatted_prompt, "gpt-4")
...
def test_anthropic_no_cot():
    test_example = EMPIRE_OF_PANTS_EXAMPLE
    formatted_prompt: list[ChatMessages] = ZeroShotSycophancyFormatter().format_example(test_example)
    formatted_prompt = get_model_specific_messages(formatted_prompt, "claude-v1")

    assert len(formatted_prompt) == 2
    prompt_sent_to_anthropic: str = convert_to_completion_str(formatted_prompt)
...
def test_anthropic_cot():
    ...
    formatted_prompt = get_model_specific_messages(formatted_prompt, "claude-v1")
    prompt_sent_to_anthropic = convert_to_completion_str(formatted_prompt)
