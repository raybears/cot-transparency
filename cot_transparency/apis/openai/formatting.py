...
def append_assistant_preferred_to_last_user(
    ...
    for msg in prompt:
        if msg.role == MessageRole.assistant_if_completion:
            ...
            new_item = StrictChatMessage(
                role=StrictMessageRole(msg.role), content=msg.content
            )
...
def append_assistant_preferred_to_next_message(
    ...
    assistant_preferred_present = Slist(prompt).any(
    ...
    if not assistant_preferred_present:
        ...
        if assistant_preferred_idx == len(prompt) - 1:
            raise ValueError(
                "Cannot format for finetuning because the assistant_preferred message is the last message"
            )
        new_messages = []
        for idx, message in enumerate(prompt):
            if idx == assistant_preferred_idx:
                ...
                new_message = StrictChatMessage(
                    role=StrictMessageRole.assistant, content=content + content_next
                )
