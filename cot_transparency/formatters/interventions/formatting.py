...
from cot_transparency.formatters.more_biases.more_reward import MoreRewardBiasedNoCOTFormatter
from cot_transparency.formatters.verbalize.formatters import StanfordNoCOTFormatter
...
def format_biased_question_non_cot_random_formatter(task: TaskOutput, formatter: Type[StageOneFormatter]) -> Prompt:
    ...
    resp = task.model_output.parsed_response
    assert resp is not None, "This should be a valid response"
    formatter_to_use = get_formatter_for_few_shot(answer_formatter=formatter, seed=read.hash())
    messages: list[ChatMessage] = add_to_final_assistant(
...
def get_formatter_for_few_shot(answer_formatter: Type[StageOneFormatter], seed: str) -> Type[StageOneFormatter]:
