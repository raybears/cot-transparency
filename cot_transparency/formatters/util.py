...
def get_few_shot_prompts(
    seed: str, n: int = -1, format_of_final: Optional[str] = None
) -> list[tuple[ChatMessage, ChatMessage, MultipleChoiceAnswer]]:
    """
    ...
    for q, a in few_shots:
        ...
        if format_of_final is not None:
            assert format_in_few_shot in a.content
            formatted_content = a.content.replace(format_in_few_shot, format_of_final)
            a = ChatMessage(role=a.role, content=formatted_content)
