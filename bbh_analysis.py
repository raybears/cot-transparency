...
def g(x):
    return glob.glob("experiments/" + x + "*")
    ...
    for idx in range(n):
        ...
        for i_bias_context in range(2):  # is biased context
            for i_cot in [False, True]:  # cot
                ...
                if bias_type == "ans_always_a":
                    ...
                    incon_to_con = int(
                        row["y_pred_prior"] != row["random_ans_idx"] and row["y_pred"] == row["random_ans_idx"]
                    )
                    con_to_incon = int(
                        row["y_pred_prior"] == row["random_ans_idx"] and row["y_pred"] != row["random_ans_idx"]
                    )
        ...
        for _ in range(2):  # is biased context
            for i_cot in [
                False,
                True,
            ]:  # NOTE: am relying on this being the same ordering as the above loop so that COT lines up correctly
                ...
                n_strong_bias_affected = int(
                    biased_context["frac_biased"] and unbiased_context["acc"] and not unbiased_context["frac_biased"]
                )
